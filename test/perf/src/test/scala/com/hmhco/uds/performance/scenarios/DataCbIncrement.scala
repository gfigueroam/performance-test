...
case object DataCbIncrement extends BaseScenario {
  val description: String = "data.cb.increment"
  val method: String = "api/v1/data.cb.increment"
  ...
  def scenario = exec(
    exec{
      session =>
        ...
        val tmpQuery = loadJSON("data.cb.increment",rmap)
